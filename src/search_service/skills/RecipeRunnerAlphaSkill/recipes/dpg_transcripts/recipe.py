...
ppt_processing_recipe = ProcessingRecipe(
    file_types=[FileType.PPTX, FileType.PPT],
...
ms_word_processing_recipe = ProcessingRecipe(
    ...
    file_processor_name=FileProcessorName.TRANSCRIPTS_MS_WORD_PAGES_PROCESSOR,
    chunking_strategy=ChunkingStrategy(
        size=1200, overlap=200, unit=ChunkingUnit.TOKENS
    ),
    error_handling=ErrorHandlingStrategy.SKIP,
...
proccessing_control = ADLSProcessingControl(
    ...
    included_directories=["dpg-cci"],
    excluded_directories=[]
)

dpg_transcripts_processing_recipe = MultiProcessingRecipe(
    id="recipe_002",
    recipes=[ms_word_processing_recipe, ppt_processing_recipe, pdf_processing_recipe],
